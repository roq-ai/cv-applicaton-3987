...
import { getCvs, deleteCvById } from 'apiSdk/cvs';
import { CvInterface } from 'interfaces/cv';

type ColumnType = ColumnDef<CvInterface, unknown>;
...
export function CvListPage(props: CvListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CvInterface>>(
    () => `/cvs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CvInterface) => {
    if (hasAccess('cv', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/cvs/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/cvs/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('cv', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/cvs/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('cv', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="cv" />
        </Flex>

        {hasAccess('cv', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/cvs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
