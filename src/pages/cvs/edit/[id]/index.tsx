...
import { getCvById, updateCvById } from 'apiSdk/cvs';
...
function CvEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CvInterface>(
    () => (id ? `/cvs/${id}` : null),
    () => getCvById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CvInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCvById(id, values);
      mutate(updated);
      resetForm();
      router.push('/cvs');
    } catch (error) {
  ...
  const formik = useFormik<CvInterface>({
    initialValues: data,
